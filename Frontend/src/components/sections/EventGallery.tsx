    ...
    <section className="bg-gray-50 px-4 py-20 sm:px-6 md:py-24 lg:py-28">
      <div className="mx-auto max-w-7xl">
        <div className="max-w-2xl mx-auto">
          <SectionHeading center={true} title="Event Gallery" subtitle="A glimpse into some of our most memorable experiences" />
        </div>

        {visibleItems.length >= 6 ? (
          <>
            <StandardGalleryGrid className="lg:hidden" items={visibleItems} onOpen={openLightbox} />
            <DesktopCollage items={visibleItems} onOpen={openLightbox} />
          </>
        ) : (
          <StandardGalleryGrid items={visibleItems} onOpen={openLightbox} />
        )}
      </div>
          ...
          <div className="relative flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl lg:grid lg:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.7fr)]">
            ...
            <div className="flex min-h-0 flex-col overflow-y-auto p-5 sm:p-7 lg:p-8">
              <div className="flex items-start justify-between gap-4">
                <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-bold text-primary">{activeItem.category}</span>
                <button ref={closeButtonRef} type="button" onClick={closeLightbox} aria-label="Close hosted event viewer"
              ...
              <div className="mt-8">
                <p className="text-sm font-semibold text-text-muted">{activeIndex + 1} / {visibleItems.length}</p>
                <h3 id="event-gallery-lightbox-title" className="mt-3 text-3xl font-bold leading-tight text-text-primary sm:text-4xl">{activeItem.title}</h3>
                ...
                {activeItem.summary && <p className="mt-6 text-base leading-7 text-text-muted">{activeItem.summary}</p>}
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className={`mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:auto-rows-[230px] lg:grid-cols-3 ${className}`}>
      {items.map((item, index) => (
        <GalleryCard key={item.id} item={item} isFeatured={item.featured} onOpen={() => onOpen(item.id)} priority={index + 1} />
      ))}
    </div>
  )
}
  ...
  return (
    <div className="mt-8 hidden h-[620px] overflow-hidden rounded-[2rem] bg-white p-1.5 shadow-lg ring-1 ring-gray-200 lg:grid lg:grid-cols-2 lg:gap-1.5">
      <div className="grid min-h-0 grid-rows-[3fr_2fr] gap-1.5">
        <GalleryCard item={leftTop} onOpen={() => onOpen(leftTop.id)} priority={1} variant="collage" />
        <GalleryCard item={leftBottom} onOpen={() => onOpen(leftBottom.id)} priority={2} variant="collage" />
      </div>
      <div className="grid min-h-0 grid-rows-[2fr_1fr_2fr] gap-1.5">
        <GalleryCard item={rightTop} onOpen={() => onOpen(rightTop.id)} priority={3} variant="collage" />
        <div className="grid min-h-0 grid-cols-2 gap-1.5">
          <GalleryCard item={rightMiddleLeft} onOpen={() => onOpen(rightMiddleLeft.id)} priority={4} variant="collage" />
          <GalleryCard item={rightMiddleRight} onOpen={() => onOpen(rightMiddleRight.id)} priority={5} variant="collage" />
        </div>
        <GalleryCard item={rightBottom} onOpen={() => onOpen(rightBottom.id)} priority={6} variant="collage" />
      </div>
    </div>
  )
}
...
const GalleryCard = ({ item, isFeatured = false, onOpen, priority, variant = "grid" }: {
  ...
  return (
    ...
    >
      ...
      />
      <div className={`absolute inset-x-0 bottom-0 bg-gradient-to-t from-gray-950/85 via-gray-950/25 to-transparent transition-all duration-300 group-hover:from-gray-950/90 ${
        isCollage ? "h-[52%] group-hover:h-[58%]" : isFeatured ? "h-[72%] group-hover:h-[78%]" : "h-[64%] group-hover:h-[70%]"
      }`} />
      <div className={`absolute inset-x-0 bottom-0 z-10 ${isCollage ? "p-5" : "p-5 sm:p-6"}`}>
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <span className="rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-primary">{item.category}</span>
          <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold text-white backdrop-blur-sm lg:hidden">{item.date}</span>
        </div>
        <h3 className={`font-bold leading-tight text-white ${isCollage ? "text-xl sm:text-2xl" : isFeatured ? "text-3xl sm:text-4xl" : "text-2xl"}`}>
          {item.title}
        </h3>
